...
describe "Items API", type: :request do
  it "can display one item result from a keyword search" do
    ...
    expect(item[:attributes][:id]).to eq(item2.id).or(eq(item3.id))
  ...
  it "search can find results from partial match" do
    ...
    expect(item[:attributes][:id]).to eq(item3.id)
